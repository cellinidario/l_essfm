...
# Original author: Christian Haeger (christian.haeger@chalmers.se), Dec 2018
# Cleaned for l_essfm: FFT-based circular convolution is now the DEFAULT
# (the original tf.nn.conv1d/cuDNN path diverged to NaN on modern GPUs; the FFT
# form matches it to ~1.5e-14 and is GPU-stable). Set LDBP_USE_CONV1D=1 to force
# the old path for debugging.  Pruning supports per-step variable tap counts
# (periodic 'target cd filter length' pattern); use a profile-aligned monotone
# pattern (more taps on the long log-steps) for single-span scenarios.
#========================================================#
...
def cconv_fft(x, h):
	""" FFT-based circular convolution, drop-in replacement for cconv().
	Same interface and delay-removal semantics, but uses FFT/IFFT only
	(no tf.nn.conv1d / cuDNN), which is GPU-stable like the L-ESSFM path.

	Circular conv + symmetric-delay removal == multiply X(f) by the filter
	frequency response of h zero-padded to N and rolled by -filter_delay.
	"""
	filter_length = int(h.shape[0])
	N = int(x.shape[1])
	filter_delay = (filter_length - 1) // 2 if filter_length % 2 == 1 else filter_length // 2 - 1

	# build complex signal xc: shape [batch, N]
	if len(x.shape) == 2:          # real signal
		xc = tf.complex(x, tf.zeros_like(x))
	else:                           # complex signal [batch, N, 2]
		xc = tf.complex(x[:, :, 0], x[:, :, 1])

	# build complex filter hc: shape [filter_length]
	if len(h.shape) == 1:           # real filter
		hc = tf.complex(h, tf.zeros_like(h))
	else:                           # complex filter [filter_length, 2]
		hc = tf.complex(h[:, 0], h[:, 1])

	# zero-pad filter to N and roll by -filter_delay (center at index 0)
	pad = tf.zeros([N - filter_length], dtype=tf.complex64)
	h_pad = tf.concat([hc, pad], axis=0)
	h_pad = tf.roll(h_pad, shift=-filter_delay, axis=0)

	Hf = tf.signal.fft(h_pad)                     # [N]
	Xf = tf.signal.fft(xc)                        # [batch, N]
	yc = tf.signal.ifft(Xf * Hf[tf.newaxis, :])   # [batch, N]

	# return shape matching original cconv
	real_sig = (len(x.shape) == 2)
	real_fil = (len(h.shape) == 1)
	if real_sig and real_fil:
		return tf.real(yc)                        # [batch, N]
	else:
		return tf.stack([tf.real(yc), tf.imag(yc)], axis=2)  # [batch, N, 2]
...
# Use the FFT-based circular convolution by DEFAULT: the original tf.nn.conv1d /
# cuDNN path diverged to NaN on modern GPUs.  The FFT form is GPU-stable and was
# validated to match conv1d to ~1.5e-14.  Set LDBP_USE_CONV1D=1 to force the old
# path (debug only).
if os.environ.get('LDBP_USE_CONV1D', '') != '1':
	cconv = cconv_fft
	print("[ldbp] using FFT-based circular convolution (GPU-stable)")
...
#========================================================#
parser = ap.ArgumentParser("python3 ldbp1.py")
...
combine_half_steps     = conf_ldbp.getboolean('combine half-steps')
...
# Diagnostic: optional gradient clipping (env LDBP_CLIP=<float>) to tame the
# fragile early iterations (LDBP starts ~ -17 dB with large gradients).
_clip = os.environ.get('LDBP_CLIP', '')
if _clip:
	_gv = optimizer.compute_gradients(mean_squared_error)
	_gv = [(tf.clip_by_value(g, -float(_clip), float(_clip)), v) for g, v in _gv if g is not None]
	train = optimizer.apply_gradients(_gv)
	print("(gradient clipping at +/-{})".format(_clip))
else:
...
# GPU diagnostic fix: allow incremental memory growth (avoid grabbing all VRAM)
# and allow soft device placement. This addresses OOM/contention-driven instability.
_gpu_config = tf.ConfigProto(allow_soft_placement=True)
_gpu_config.gpu_options.allow_growth = True
sess = tf.Session(config=_gpu_config)
